...
test.describe("Parabank challenge - Complex scenario flow", () => {
  test("Login - Invalid user - Simple scenario to test structure", async ({
    ...
    await expect(customerLogin.errorMessageTitle).toHaveText(
      errorMessages.invalidLogin.title,
    );
    await expect(customerLogin.errorMessageBody).toHaveText(
      errorMessages.invalidLogin.body,
    );
    await expect(customerLogin.errorMessageBody).toHaveCSS(
      "color",
      "rgb(255, 0, 0)",
    );
  });
  ...
  test("Login - Valid user - Transfer amount and check account details", async ({
    ...
    await customerLogin.login(registeredUser.username, registeredUser.password);
